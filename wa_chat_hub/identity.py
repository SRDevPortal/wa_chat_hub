...
def expire_conversation_verification(
    ...
    *,
    ...
    """Reset an expired verified conversation without removing its patient link."""
...
def reconcile_conversation_identity(
    ...
    *,
    ...
    patient_party_type = str(party_types.get("Patient") or "").strip()
    lead_party_type = str(
        party_types.get("CRM Lead") or party_types.get("Lead") or ""
    ).strip()
    unknown_party_type = str(routing_policy.get("unknown_party_type") or "").strip()
    if not all((
        verified_status, matched_status, unverified_status,
        patient_party_type, lead_party_type, unknown_party_type,
    )):
    ...
    if patient and frappe.db.exists("Patient", patient):
        identity_status = (
            verified_status
            if verified or getattr(convo, "identity_status", None) == verified_status
            else matched_status
        )
        values.update(
            {
                "linked_patient": patient,
                "party_type": patient_party_type,
