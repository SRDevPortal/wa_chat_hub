...
from wa_chat_hub.maintenance import attachment_indexes as indexes
...
class TestAttachmentIndexes(TestCase):
    def setUp(self):
        self.db = MagicMock()
    ...
    def test_online_build_and_repeated_execution(self):
        self.assertEqual(indexes.ensure_attachment_index()["status"], "created")
        ...
        self.assertEqual(indexes.ensure_attachment_index()["status"], "already_present")
    ...
    def test_equivalent_longer_index_under_another_name_is_reused(self):
        ...
        self.assertEqual(indexes.ensure_attachment_index()["index"], "existing_lookup")
    ...
    def test_conflicting_name_fails_without_dropping_an_index(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "unexpected definition"):
            indexes.ensure_attachment_index()
        self.db.sql_ddl.assert_not_called()
    ...
    def test_missing_table_is_safe_during_installation(self):
        ...
        self.assertEqual(indexes.ensure_attachment_index()["status"], "table_absent")
        self.db.sql_ddl.assert_not_called()
