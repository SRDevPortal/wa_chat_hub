...
from wa_chat_hub.api.actions import create_crm_lead_from_conversation, create_lead_from_conversation
...
class TestCreateLeadAction(TestCase):
    def test_reuses_erpnext_lead_phone_match_and_links_current_conversation(self):
        convo = frappe._dict(
            name="CONV-001",
            contact="CONTACT-001",
            channel_account="ACCOUNT-001",
        ...
        with (
            ...
            patch("wa_chat_hub.api.actions.safe_ai_exists", return_value=True),
            patch("wa_chat_hub.api.actions.conversation_update_lock", return_value=nullcontext()),
            patch("wa_chat_hub.api.actions._find_existing_lead_by_phone", return_value=("Lead", "LEAD-001")),
            patch("wa_chat_hub.api.actions._create_lead_for_inbound") as create_lead,
            patch("wa_chat_hub.api.actions._finalize_lead_after_inbound") as finalize_lead,
            patch("wa_chat_hub.api.actions.frappe", frappe_mock),
        ):
            result = create_lead_from_conversation.__wrapped__()

        self.assertFalse(result["result"]["created"])
        self.assertEqual(result["result"]["doctype"], "Lead")
        self.assertEqual(result["result"]["name"], "LEAD-001")
        create_lead.assert_not_called()
        finalize_lead.assert_called_once_with("CONV-001", "Lead", "LEAD-001", convo=convo)
        self.assertEqual(frappe_mock.db.set_value.call_count, 2)

    def test_creates_erpnext_lead_with_current_channel_account(self):
        convo = frappe._dict(
            name="CONV-001",
            contact="CONTACT-001",
            channel_account="ACCOUNT-001",
        ...
        with (
            ...
            patch("wa_chat_hub.api.actions.safe_ai_exists", return_value=True),
            patch("wa_chat_hub.api.actions.conversation_update_lock", return_value=nullcontext()),
            patch("wa_chat_hub.api.actions._find_existing_lead_by_phone", return_value=None),
            patch("wa_chat_hub.api.actions._create_lead_for_inbound", return_value="LEAD-NEW") as create_lead,
            patch("wa_chat_hub.api.actions._finalize_lead_after_inbound"),
            patch("wa_chat_hub.api.actions.frappe", frappe_mock),
        ):
            result = create_lead_from_conversation.__wrapped__()
