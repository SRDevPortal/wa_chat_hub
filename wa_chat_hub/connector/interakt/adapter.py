...
def split_interakt_phone(phone_number: str, default_country_code: str = "+91") -> Tuple[str, str]:
    ...
    inferred = _infer_country_code_from_digits(digits)
    if inferred:
        inferred_country, local_number = inferred
        return f"+{inferred_country}", local_number
...
def _infer_country_code_from_digits(digits: str) -> Tuple[str, str] | None:
    for length in (3, 2, 1):
        country = digits[:length]
        local_number = digits[length:]
        if country in KNOWN_COUNTRY_CODES and 6 <= len(local_number) <= 12:
            return country, local_number
    return None
