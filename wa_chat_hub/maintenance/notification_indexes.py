"""Indexes for notification delivery evidence. Run outside business transactions."""
...
INDEX_NAME = "idx_chat_message_event_direction_modified"
FIELDS = ("provider_event_id", "direction", "modified")
...
def ensure_notification_event_index() -> dict:
    ...
    if not all(frappe.db.has_column(DOCTYPE, field) for field in FIELDS):
        raise RuntimeError("Chat Message delivery fields are missing; synchronize its schema first")
    ...
    try:
        ...
        frappe.db.sql_ddl(
            ...
            "(`provider_event_id`, `direction`, `modified`), ALGORITHM=INPLACE, LOCK=NONE"
        )
    finally:
        frappe.db.sql("SET SESSION lock_wait_timeout = %s", (previous_wait,))

    if not _equivalent_index(_indexes()):
        raise RuntimeError("Notification event index was not found after online DDL")
